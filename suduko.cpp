...
bool validPlace9x9(int arr[9][9], int row, int col, int n)
{
...
std::vector<int> findPlaceables(int arr[9][9], int row, int col) {
	...
	for (int n = 1; n <= 9; n++)
		if (validPlace9x9(arr, row, col, n)) placebles.push_back(n);
	return placebles;
}
...
int main(int argc, char** argv)
{
	...
	if (solveSudoku9x9(board, 0, 0)) cout << "successfully solved !!!\n" << std::endl;
	printSudoku9x9(board);
	if (solveSudoku9x9(board2, 0, 0)) cout << "successfully solved the board 2!!!\n" << std::endl;
